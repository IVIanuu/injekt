...
private fun ConeKotlinType.contains(
    predicate: (ConeKotlinType) -> Boolean,
    visited: SmartSet<ConeKotlinType>?
): Boolean {
    ...
    return when (this) {
        is ConeFlexibleType -> lowerBound.contains(predicate, visited) || upperBound.contains(
            predicate,
            visited
        )
        ...
        else -> typeArguments.any {
            it is ConeKotlinTypeProjection && it.type.contains(
                predicate,
                visited
            )
        }
...
fun ConeClassLikeType.withArguments(typeArguments: Array<out ConeTypeProjection>): ConeClassLikeType =
    when (this) {
