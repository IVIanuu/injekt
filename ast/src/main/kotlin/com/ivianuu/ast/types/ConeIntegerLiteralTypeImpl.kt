...
class ConeIntegerLiteralTypeImpl : ConeIntegerLiteralType {
    ...
    override fun getApproximatedType(expectedType: ConeKotlinType?): ConeClassLikeType {
        val approximatedType =
            when (val expectedTypeForApproximation = expectedType?.lowerBoundIfFlexible()) {
    ...
    companion object {
         ...
         */
        private fun findCommonSuperTypeOrIntersectionType(
            types: Collection<SimpleTypeMarker>,
            mode: Mode
        ): SimpleTypeMarker? {
            if (types.isEmpty()) return null
            @Suppress("UNCHECKED_CAST")
            return types.reduce { left: SimpleTypeMarker?, right: SimpleTypeMarker? ->
                fold(
                    left,
                    right,
                    mode
        ...
        private fun fold(
            left: SimpleTypeMarker?,
            right: SimpleTypeMarker?,
            mode: Mode
        ): SimpleTypeMarker? {
        ...
        private fun fold(
            left: ConeIntegerLiteralType,
            right: ConeIntegerLiteralType,
            mode: Mode
