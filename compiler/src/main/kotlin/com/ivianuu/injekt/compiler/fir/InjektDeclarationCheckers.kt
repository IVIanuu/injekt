...
@file:OptIn(UnsafeCastFunction::class, SymbolInternals::class,
  InternalDiagnosticFactoryMethod::class, UnsafeCastFunction::class, UnsafeCastFunction::class
)
...
private fun checkSpreadTypeParameters(
  ...
  val spreadParameters = typeParameters.filter {
    it.hasAnnotation(InjektFqNames.Spread, context.session)
  }
  if (spreadParameters.size > 1)
    spreadParameters
      ...
      .forEach {
        reporter.report(
          it.getAnnotationByClassId(InjektFqNames.Spread, context.session)!!.source!!,
...
private fun isValidOverride(
  ...
  for ((index, overriddenTypeParameter) in overriddenTypeParameters.withIndex()) {
    ...
    if (typeParameter.hasAnnotation(InjektFqNames.Spread, session) !=
      overriddenTypeParameter.hasAnnotation(InjektFqNames.Spread, session))
      return false
  }
