...
data class InjektClassifier(
  ...
  override fun equals(other: Any?): Boolean = (other is InjektClassifier) && fqName == fqName
...
fun ClassifierDescriptor.toInjektClassifier(ctx: Context): InjektClassifier =
  ctx.cache.cached("classifier_ref", this) {
    val expandedType = (original as? TypeAliasDescriptor)?.underlyingType
      ?.toTypeRef(ctx)

    val isTag = hasAnnotation(InjektFqNames.Tag) || fqNameSafe == InjektFqNames.Composable.asSingleFqName()
