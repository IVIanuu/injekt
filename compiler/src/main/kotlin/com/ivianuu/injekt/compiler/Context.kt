...
class Context(val module: ModuleDescriptor, val trace: BindingTrace?) : TypeCheckerContext {
  ...
  val nullableNothingType by lazy(LazyThreadSafetyMode.NONE) { module.builtIns.nullableNothingType.toInjektType(ctx = ctx) }
  val anyType by lazy(LazyThreadSafetyMode.NONE) { module.builtIns.anyType.toInjektType(ctx = ctx) }
