...
import com.ivianuu.injekt.compiler.ModuleDescriptor
...
import com.ivianuu.injekt.compiler.isAssignable
        ...
        if (binding in chain) {
            ...
            if (relevantSubchain.any {
                    it is ProviderBindingNode ||
                            it.type.classifier.fqName.startsWith("kotlin.Function")
                }) return
            error(
        ...
        if (type.isFunction && type.typeArguments.last().let {
            ...
            if (childComponentType !in existingComponents) {
                val componentImpl = componentImplFactory(
                    childComponentType,
                    owner.contextTreeNameProvider("C"),
                    owner
                )
...
private fun String?.orUnknown(): String = this ?: "unknown origin"
