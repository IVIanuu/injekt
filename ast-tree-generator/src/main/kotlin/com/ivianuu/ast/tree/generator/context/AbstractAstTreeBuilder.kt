...
abstract class AbstractAstTreeBuilder {
    companion object {
    ...
    val elements = mutableListOf(baseAstElement)
    val intermediateBuilders = mutableListOf<IntermediateBuilder>()

    protected fun element(
        name: String,
        kind: Element.Kind,
...
fun type(packageName: String?, type: String, exactPackage: Boolean = false): Type {
    val realPackage =
        if (exactPackage) packageName else packageName?.let { "com.ivianuu.$it" }
