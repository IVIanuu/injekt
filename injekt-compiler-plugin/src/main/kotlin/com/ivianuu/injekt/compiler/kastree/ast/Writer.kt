package com.ivianuu.injekt.compiler.kastree.ast
...
open class Writer(val app: Appendable = StringBuilder()) : Visitor() {
  ...
  override fun visit(v: Node?, parent: Node) {
    v?.apply {
      ...
      if (replacedSource != null) append(replacedSource)
        ...
        is Node.Expr.While -> {
          if (!doWhile) append("while (").also { children(expr) }.append(") ")
            .also { children(body) }
          else append("do ").also { children(body) }.append(" while (").also { children(expr) }
            .append(')')
        }
  ...
  companion object {
    ...
    fun write(v: Node) =
      write(v, StringBuilder()).toString()

    fun <T : Appendable> write(v: Node, app: T) =
      app.also { Writer(it).write(v) }
  }
}
