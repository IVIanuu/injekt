...
class ReaderTypeRemapper(private val injektContext: InjektContext) : TypeRemapper {
    ...
    override fun remapType(type: IrType): IrType {
        ...
        if (!type.isFunction() && !type.isSuspendFunction() &&
                !type.isKFunction() && !type.isKSuspendFunction()) return underlyingRemapType(type)
        ...
        val extraArg = makeTypeProjection(
            injektContext.injektSymbols.context.defaultType,
            Variance.INVARIANT
        )
        val newIrArguments =
        ...
        val classifier = injektContext.symbolRemapper.getReferencedClassifier(
            when {
                type.isFunction() -> injektContext.tmpFunction(oldIrArguments.size)
                type.isKFunction() -> injektContext.tmpKFunction(oldIrArguments.size)
                type.isSuspendFunction() -> injektContext.tmpSuspendFunction(oldIrArguments.size)
                type.isKSuspendFunction() -> injektContext.tmpSuspendKFunction(oldIrArguments.size)
    ...
    private fun underlyingRemapType(type: IrSimpleType): IrType {
        val classifier = injektContext.symbolRemapper.getReferencedClassifier(type.classifier)
    ...
    private fun IrTypeAbbreviation.remapTypeAbbreviation() =
        IrTypeAbbreviationImpl(
            injektContext.symbolRemapper.getReferencedTypeAlias(typeAlias),
            hasQuestionMark,
