...
class CallContextTest {
  @Test fun testSuspendCannotBeRequestedFromNonSuspend() = codegen(
    ...
    compilationShouldHaveFailed("injectable injekt.integrationtests.bar() of type injekt.integrationtests.Bar for parameter x of function injekt.inject is a suspend function but current call context is composable")
  }
  ...
  @Test fun testComposableDependencyCannotBeRequestedFromNonComposable() = singleAndMultiCodegen(
    ...
    compilationShouldHaveFailed("injectable injekt.integrationtests.bar() of type injekt.integrationtests.Bar for parameter x of function injekt.inject is a composable function but current call context is suspend")
  }
  ...
  @Test fun testSuspendLambdaCanRequestSuspendDependencyInDefaultContext() =
  ...
  @Test fun testComposableLambdaCanRequestComposableDependencyInDefaultContext() =
    singleAndMultiCodegen(
      """
        @Provide @Composable fun foo() = Foo()
        @Provide fun bar(foo: Foo) = Bar(foo)
  ...
  @Test fun testCanRequestComposableDependencyFromNonComposableFunctionInComposableLambda() =
    singleAndMultiCodegen(
      """
        @Provide @Composable fun foo() = Foo()
        @Provide fun lazyBar(): @Composable () -> Bar = { Bar(inject(foo())) }
      """,
      """
        fun invoke() = inject<@Composable () -> Bar>({ TODO() })
      """,
      config = { withCompose() }
    ) {
      runComposing {
  ...
  @Test fun testSuspendCanBeRequestedFromInlineProviderInSuspendContext() = singleAndMultiCodegen(
    """
      @Provide suspend fun foo() = Foo()
      suspend inline fun createFoo(provider: () -> Foo = inject) = provider()
    """,
    """
  ...
  @Test fun testComposableCanBeRequestedFromInlineProviderInComposableContext() = singleAndMultiCodegen(
    """
      @Provide @Composable fun foo() = Foo()
      @Composable inline fun createFoo(provider: () -> Foo = inject) = provider()
    """,
    """
  ...
  @Test fun testCannotRequestSuspendDependencyInDefaultValueOfFunction() = singleAndMultiCodegen(
    ...
    compilationShouldHaveFailed("injectable injekt.integrationtests.foo() of type injekt.integrationtests.Foo for parameter x of function injekt.inject is a suspend function but current call context is default")
  }
