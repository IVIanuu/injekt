...
class KotlinCompilation {
    ...
    var kotlinReflectJar: File? by default {
        findInHostClasspath(
            hostClasspaths, "kotlin-reflect.jar",
    ...
    var kotlinScriptRuntimeJar: File? by default {
        findInHostClasspath(
            hostClasspaths, "kotlin-script-runtime.jar",
    ...
    var kotlinStdLibCommonJar: File? by default {
        findInHostClasspath(
            hostClasspaths, "kotlin-stdlib-common.jar",
    ...
    private fun stubsAndApt(sourceFiles: List<File>): ExitCode {
        ...
        val kaptLogger =
            MessageCollectorBackedKaptLogger(kaptOptions.build(), compilerMessageCollector)
        ...
        if (!isJdk9OrLater()) {
            try {
                ...
                (ClassLoader.getSystemClassLoader() as URLClassLoader).addUrl(
        ...
        val k2JvmArgs = commonK2JVMArgs().also {
            ...
            it.pluginClasspaths =
                (it.pluginClasspaths ?: emptyArray()) + arrayOf(getResourcesPath())
        }
    ...
    private fun compileKotlin(sourceFiles: List<File>): ExitCode {
        ...
        val k2JvmArgs = commonK2JVMArgs().also { jvmArgs ->
            jvmArgs.pluginClasspaths =
                (jvmArgs.pluginClasspaths ?: emptyArray()) + arrayOf(getResourcesPath())
